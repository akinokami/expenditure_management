...
final kExpenditure = LinkedHashMap<DateTime, List<ExpenditureModel>>(
  equals: isSameDay,
  hashCode: getHashCode,
)..addAll(Map.fromIterable(Get.find<HomeController>().expList));

// final _kEventSource = Map.fromIterable(
//     List.generate(Get.find<HomeController>().expList.length, (index) => index),
...
// final _kExpmSource = Map.fromIterable(
//     List.generate(Get.find<HomeController>().expList.length, (index) => index),
//     key: (item) => DateTime.utc(kFirstDay.year, kFirstDay.month, item * 5),
//     value: (item) => List.generate(
//         item % 4 + 1, (index) => Event('Event $item | ${index + 1}')))
